...
const mockTrainingPosts = [
  { id: 1, title: "Java Workshop", description: "Learn Java basics", eligibility: "CS/IT", openings: 10, location: "Remote" },
];

const TrainingForm = () => {
  const [searchTerm, setSearchTerm] = useState("");

  const filteredPosts = mockTrainingPosts.filter(
    (post) =>
      post.title.toLowerCase().includes(searchTerm.toLowerCase()) ||
      post.description.toLowerCase().includes(searchTerm.toLowerCase())
  );

  const handleApply = (postId: number) => alert(`Applied for training post ID: ${postId}`);

  return (
    <div className="space-y-6">
      <h1 className="text-3xl font-bold tracking-tight">Apply for Training / Skill Workshop</h1>
